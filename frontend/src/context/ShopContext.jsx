        ...
        try {
            ...
            if (response.data.success) {
                if (response.data.products.length === 0) {
                    // Use fallback products if backend is empty
                    setProducts(fallbackProducts)
                } else {
                    setProducts(response.data.products.reverse())
                }
            } else {
                // Use fallback products if backend request fails
                setProducts(fallbackProducts)
            }

        } catch (error) {
            console.log('Backend not available, using fallback products:', error)
    ...
    const getUserCart = async (token) => {
        try {
            ...
            if (response.data.success) {
                ...
                // Clean invalid cart items (products that don't exist)
                const cleanedCartData = {};
                
                // Wait for products to be loaded before cleaning cart
                setTimeout(() => {
